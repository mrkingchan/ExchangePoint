...
-(void)maProfileWithModel:(Y_KLineModel *)model
{
    if(self.targetLineStatus != Y_StockChartTargetLineStatusKDJ)
    {
        ...
        _DIFLabel.text = [NSString stringWithFormat:@"  DIF：%.3f ",model.DIF.floatValue];
        _DEALabel.text = [NSString stringWithFormat:@"  DEA：%.3f",model.DEA.floatValue];
        _MACDLabel.text = [NSString stringWithFormat:@"  MACD：%.3f",model.MACD.floatValue];
    } else {
        _accessoryDescLabel.text = @" KDJ(9,3,3)";
        _DIFLabel.text = [NSString stringWithFormat:@"  K：%.3f ",model.KDJ_K.floatValue];
        _DEALabel.text = [NSString stringWithFormat:@"  D：%.3f",model.KDJ_D.floatValue];
        _MACDLabel.text = [NSString stringWithFormat:@"  J：%.3f",model.KDJ_J.floatValue];
    }
