...
-(void)maProfileWithModel:(Y_KLineModel *)model
{
    ...
    _volumeDescLabel.text = [NSString stringWithFormat:@" 成交量(7,30):%.3f ",model.Volume];
 
    _VolumeMA7Label.text = [NSString stringWithFormat:@"  MA7：%.3f ",model.Volume_MA7.floatValue];
    _VolumeMA30Label.text = [NSString stringWithFormat:@"  MA30：%.3f",model.Volume_MA30.floatValue];
}
