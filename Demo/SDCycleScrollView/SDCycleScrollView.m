...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSUInteger count = self.imageURLStringsGroup.count;
    for (NSUInteger i = 0; i < count; i++) {
        MJPhoto *photo = [[MJPhoto alloc] init];
        // 设置图片的路径
        photo.url = [NSURL URLWithString:self.imageURLStringsGroup[i]];
        // 设置来源于哪一个UIImageView
    ...
    // 3.设置默认显示的图片索引
    browser.currentPhotoIndex = indexPath.item % self.imagePathsGroup.count;
