...
- (instancetype)initWithArray:(NSArray *)titleArray complete:(void (^)(NSInteger))complete {
    if (self = [super initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, 45)]) {
...
- (void)setUpChildViews {
    CGFloat itemW = [UIScreen mainScreen].bounds.size.width / _titleArray.count;
    for (int i = 0; i < _titleArray.count ; i ++) {
        ...
        [button mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.left.equalTo(subView.mas_left).offset(itemW /2 - 45);
        }];
...
- (void)buttonAction:(UIButton *)button {
    ...
    [UIView animateWithDuration:0.3 animations:^{
        _line.frame =  CGRectMake(index * itemW + (itemW/ 2 - 17.5),43.5 , 35
                                  , 1.5);
    }];
    if (_complete) {
        _complete(index);
